...
(function () {
  ...
  let input = null;
  let list = null;
  let items = []; // { title, subtitle, action, meta }
  let filtered = [];
  let selectedIndex = 0;
  let summaries = [];
  ...
  // Expose for nav button / other callers
  ...
  function filterItems(query) {
    ...
    return items
      .map((it) => {
        const hay = it._haystack || `${it.title} ${it.subtitle || ""}`.toLowerCase();
  ...
  // navigator.platform or userAgent string. Covers macOS on Intel and Apple Silicon.
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const isMac =
    (ua && ua.platform && ua.platform.toLowerCase().includes("mac")) ||
    /mac/i.test(legacy);
